...
class LFCC(torch_nn.Module):
    ...
    def __init__(self, fl, fs, fn, sr, filter_num, 
                 ...
                 filter_type=None, filter_scale='constant'):
        """ Initialize LFCC
        ...
        if filter_type == 'gammatone':
            for idx in range(filter_num):
                center = filter_bands[idx+1]
                b, a = nii_dsp.gammatone(center, 'iir', fs=sr)
                w, h = freqz(b, a, self.num_fft_bins)
                filter_bank[:, idx] = torch.from_numpy(abs(h))
        elif filter_type == 'bark_fbanks':
            bark_fbanks_mat, bark_freqs = bark_filter_banks(
                nfilts=filter_num,
                nfft=fn,
                fs=fs,
                low_freq=min_freq,
            ...
            filter_bank = torch.from_numpy(bark_fbanks_mat.T)
        elif filter_type == 'gammatone_fbanks':
            gammatone_fbanks_mat, bark_freqs = gammatone_filter_banks(
                nfilts=filter_num,
                nfft=fn,
                fs=fs,
                low_freq=min_freq,
            ...
            filter_bank = torch.from_numpy(gammatone_fbanks_mat.T)
        elif filter_type == 'linear_fbanks':
            linear_fbanks_mat, bark_freqs = linear_filter_banks(
                nfilts=filter_num,
                nfft=fn,
                fs=fs,
                low_freq=min_freq,
            ...
            filter_bank = torch.from_numpy(linear_fbanks_mat.T)
        elif filter_type == 'mel_fbanks':
            mel_fbanks_mat, bark_freqs = mel_filter_banks(
                nfilts=filter_num,
                nfft=fn,
                fs=fs,
                low_freq=min_freq,
            ...
            filter_bank = torch.from_numpy(mel_fbanks_mat.T)
        else:
            for idx in range(filter_num):
                filter_bank[:, idx] = trimf(
                    f, [filter_bands[idx], 
        ...
        print('filter_bank len', filter_bank.shape)
...
class GFB(LFCC):
    ...
    def __init__(self, fl, fs, fn, sr, filter_num, 
                 with_energy=False, with_emphasis=True,
                 with_delta=False, filter_type='', 
                 filter_scale='constant'):
        """ Initialize GFB
        ...
        -----
          ...
          with_delta: bool, (default True), whether use delta and delta-delta
        """
        super(GFB, self).__init__(fl, fs, fn, sr, filter_num, with_energy,
                                 with_emphasis, with_delta, flag_for_LFB=True, 
                                  filter_type=filter_type, filter_scale=filter_scale)
        return
    ...
    def forward(self, x):
        ...
        """
        return super(GFB, self).forward(x)
