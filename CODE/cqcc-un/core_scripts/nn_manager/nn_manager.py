...
from core_scripts.nn_manager.nn_manager_parallel import ParallelModel
...
# fraction_to_reserve = 0.9  # You can adjust this value as needed

# # Get the current GPU device
# device = torch.device('cuda:3')

# # Set the memory fraction for the current device
# torch.cuda.set_per_process_memory_fraction(fraction_to_reserve, device=device)
...
def f_run_one_epoch(args,
    ...
    for data_idx, (data_in, data_tar, data_info, idx_orig) in enumerate(data_loader):
        ...
        for idx, data_seq_info in enumerate(data_info):
            ...
            monitor.log_loss(
                loss_values, loss_flags, (end_time-start_time) / batchsize, 
                data_seq_info, idx_orig.numpy()[idx], epoch_idx
            )
...
def f_train_wrapper(args, pt_model, loss_wrapper, device, \
    ...
    if torch.cuda.device_count() > 1 and args.multi_gpu_data_parallel:
        ...
        pt_model = nn.parallel.DistributedDataParallel(pt_model)
        pt_model = ParallelModel(pt_model)
    else:
    ...
    # print
    nii_op_display_tk.print_log_head()
    ...
    for epoch_idx in range(start_epoch, epoch_num):
        ...
        # run one epoch
        f_run_one_epoch(
            args, pt_model, loss_wrapper, device, monitor_trn, train_data_loader,
            epoch_idx, optimizer, normtarget_f, train_dataset_wrapper
        )
        ...
        if val_dataset_wrapper is not None:
            ...
            if hasattr(pt_model, 'validation'):
                ...
                nii_display.f_print(mes, 'warning')
                nii_display.f_print('validation')
            if hasattr(pt_model, 'flag_validation'):
                pt_model.flag_validation = True
                nii_display.f_print('flag_validation')

            with torch.no_grad():
                f_run_one_epoch(
                    args, pt_model, loss_wrapper, device, monitor_val, val_data_loader, 
                    epoch_idx, None, normtarget_f, val_dataset_wrapper
                )
        ...
        if not args.not_save_each_epoch:
            ...
            torch.save(tmp_dic, './epochs/' + tmp_model_name)
            if args.verbose == 1:
